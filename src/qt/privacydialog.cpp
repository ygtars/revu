                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zREVU ought to be enough for anybody." - Bill Gates, 2017
    ui->zREVUpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzREVUSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzREVU->setEnabled(false);
        ui->pushButtonMintzREVU->setToolTip(tr("zREVU is currently disabled due to maintenance."));

        ui->pushButtonSpendzREVU->setEnabled(false);
        ui->pushButtonSpendzREVU->setToolTip(tr("zREVU is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zREVUpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzREVU_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zREVU is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zREVU...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zREVU in ") +
...
void PrivacyDialog::on_pushButtonSpendzREVU_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zREVU is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zREVU
        sendzREVU();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zREVU
    sendzREVU();
}

void PrivacyDialog::on_pushButtonZRevuControl_clicked()
{
    ZRevuControlDialog* zREVUControl = new ZRevuControlDialog(this);
    zREVUControl->setModel(walletModel);
    zREVUControl->exec();
}

void PrivacyDialog::setZRevuControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzREVUSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzREVU()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Revu Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zREVUpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zREVU is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " REVU </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zREVU</b>";
    ...
    // use mints from zREVU selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZRevuControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZRevuControlDialog::GetSelectedMints();
    }
    ...
    // Clear zrevu selector in case it was used
    ZRevuControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zREVU Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Revu, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zREVU Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zREVU </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zREVU "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zREVU "));
    ui->labelzREVUAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzREVUSyncStatus->setVisible(fShow);
}
