...
#include "ui_zrevucontroldialog.h"
...
std::list<std::string> ZRevuControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZRevuControlDialog::listMints;

ZRevuControlDialog::ZRevuControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZRevuControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZRevuControlDialog::~ZRevuControlDialog()
{
    delete ui;
}

void ZRevuControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRevuControlDialog::updateList()
{
...
void ZRevuControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRevuControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZRevu_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRevuControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZRevuControlDialog::GetSelectedMints()
{
...
void ZRevuControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
